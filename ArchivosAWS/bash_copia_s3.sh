...
manejar_error() {
    ...
    sudo aws logs put-log-events --region us-east-1 --log-group-name /ec2/bash --log-stream-name i-09d428e7805f93f2a --log-events timestamp=$timestamp,message="error"
    exit 1
}
...
archivos=$(find /install/PPR/Reportes/Poblacion_General/ -mmin -60 -type f)
...
for archivo in $archivos; do
  ...
  sudo aws s3 cp "$archivo" s3://auna-prd-integracion-s3/sap-insurance/Reportes/Poblacion_General/2023/ --acl public-read


  fecha_fin=$(date '+%Y-%m-%d %H:%M:%S')

  #generando variables para mensaje
  inicio="Iniciando copia $archivo - $fecha_inicio - "
  fechamodiarchivo="Fecha de modificacion archivo: $fecha_modificacion - "
  tamanhoarchivo="Tamanho: $tamanho MB - "
  fin="Finalizando copia - $fecha_fin"
  ...
  sudo aws logs put-log-events --region us-east-1 --log-group-name /ec2/bash --log-stream-name i-09d428e7805f93f2a --log-events timestamp=$timestamp,message="OK"
